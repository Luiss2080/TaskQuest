...
export default function Logros() {
  ...
  const logros = [
    ...
    { id: 3, title: 'Máquina', desc: 'Completa 5 misiones', unlocked: completedTasks >= 5, icon: <Zap className="w-8 h-8" />, color: 'text-rose-500' },
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {logros.map(a => (
          <div key={a.id} className={`p-6 rounded-[2rem] shadow-xl border flex items-center gap-6 transition-all ${a.unlocked ? 'bg-rose-900 border-white/10' : 'opacity-40 grayscale bg-transparent border-dashed border-2 border-white/10'}`}>
            <div className={`p-5 rounded-3xl ${a.unlocked ? `bg-rose-950 shadow-inner border border-white/5 ${a.color}` : 'bg-black/20 text-white/30'}`}>
              {a.icon}
            </div>
